...
async function addToCart(pid,qty=1){
  const res=await fetch('/cart/add',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`product_id=${pid}&qty=${qty}`});
  const d=await res.json();
  if(d.success){
    const r2=await fetch('/cart/update',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`product_id=${pid}&qty=${(cart.find(x=>x.id==pid)?.qty||0)+qty}`});
...
async function cartUpdate(pid,qty){
  const res=await fetch('/cart/update',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`product_id=${pid}&qty=${qty}`});
  const d=await res.json();
...
si?.addEventListener('input',function(){
  ...
  st=setTimeout(async()=>{
    const r=await fetch('/products/search?q='+encodeURIComponent(q));
    const d=await r.json();
    if(!d.results?.length){if(sd)sd.style.display='none';return;}
    if(sd){
      sd.innerHTML=d.results.slice(0,5).map(p=>`<a href="/products/detail/${p.id}" class="search-item"><div class="search-item-grade">${p.grade||'?'}</div><div style="flex:1;min-width:0"><div style="font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${p.name}</div><div style="font-family:var(--font-mono);font-size:10px;color:var(--text-hint)">${p.series||''}</div></div><div style="font-family:var(--font-display);font-size:15px;color:var(--gold);flex-shrink:0;margin-left:8px">${Number(p.price).toLocaleString('vi-VN')}đ</div></a>`).join('')+`<a href="/products?search=${encodeURIComponent(q)}" class="search-item" style="justify-content:center;color:var(--text-hint);font-family:var(--font-mono);font-size:11px">Xem tất cả &rarr;</a>`;
