...
class News
{
    ...
    {
        $stmt = $this->db->prepare("SELECT * FROM news WHERE is_published = 1 ORDER BY created_at DESC LIMIT :limit");
        $stmt->bindValue(':limit', $limit, PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    public function create(array $data): bool
    {
        $stmt = $this->db->prepare("
            INSERT INTO news (title, slug, summary, content, image_path, is_published, created_at, updated_at) 
            VALUES (:title, :slug, :summary, :content, :image_path, :is_published, NOW(), NOW())
        ");
        return $stmt->execute([
            ':title'        => $data['title'],
            ':slug'         => $data['slug'],
    ...
    public function update(int $id, array $data): bool
    {
        $stmt = $this->db->prepare("
            UPDATE news 
            SET title = :title, slug = :slug, summary = :summary, content = :content, image_path = :image_path, is_published = :is_published, updated_at = NOW() 
            WHERE id = :id
        ");
        return $stmt->execute([
            ':title'        => $data['title'],
