...
<style>
.tab {
  padding: 9px 16px; font-family: var(--font-m); font-size: 10px; letter-spacing: 0.08em;
  color: var(--text-2); border-right: 1px solid var(--border); transition: all 0.15s;
  display: flex; align-items: center; gap: 7px; white-space: nowrap;
}
.tab:last-child { border-right: none; }
.tab:hover { background: var(--bg-hover); color: var(--text-1); }
.tab.active { background: rgba(200,168,90,0.1); color: var(--gold); }
.tab-count {
  background: var(--border); border-radius: 10px; padding: 1px 6px;
  font-size: 9px; color: var(--text-2);
}
.tab.active .tab-count { background: rgba(200,168,90,0.2); color: var(--gold); }
</style>
...
async function updateStatus(orderId, status, btn) {
  ...
  const res = await fetch('/admin/orders/status', {
    method: 'POST',
